...
DatasetName: Mixed Income Averages (HNI) - Full Population
DatasetCode: WID_NACCAV_MIXED_HNI_P0P100
Description: Average mixed income across all population percentiles for HNI (High Net Income) groups. Mixed income represents income generated by unincorporated enterprises where labor and capital components cannot be separately identified. Part of national accounts components that help understand the structure of economic production and income distribution.
Unit: >
    VEF; percentile p0p100; accmhni992
Source:
  OrganizationCode: WID
  QueryCode: wid_all_data
DatasetProcessorFile: sspi_flask_app/api/core/datasets/wid/wid_naccav_mixed_hni_p0p100.py
---
# Consumption Of Fixed Capital Attributable To Mixed Income

## Overview

This dataset from the World Inequality Database (WID) measures **consumption of fixed capital attributable to mixed income** across income and wealth distributions. Average income or wealth between two percentiles. When the associated percentile is of the form 'pX', intermediary average returns the average between percentile pX and the next consecutive percentile. When the associated percentile is of the form 'pXpY', the variable returns the average between percentiles pX and pY.

## Variable Details

- **WID Variable Code**: `accmhni992`
