...
EXEMPT_PREFIXES_GET = (
    'api_bp.sync_bp.static',
)


def test_sync_routes_are_protected(app, client):
    """Make GET/POST requests to ensure sync routes are protected appropriately."""
    for endpoint, (route, methods) in get_route_rules(app, "api_bp.sync_bp").items():
