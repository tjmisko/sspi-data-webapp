...
EXEMPT_PREFIXES_GET = (
    'api_bp.load_bp.static',
)


def test_load_routes_are_protected(app, client):
    """Make GET/POST requests to ensure load routes are protected appropriately."""
    for endpoint, (route, methods) in get_route_rules(app, "api_bp.load_bp").items():
