from sspi_flask_app.api.resources.utilities import extrapolate_backwards


def test_extrapolate_backwards_single_series():
    data = [
        {"CountryCode": "USA", "IndicatorCode": "GDP", "Year": 2000, "Value": 100}
    ]
    extrapolate_backwards(data, 1997)
...
def test_no_extrapolation_needed():
    ...
    extrapolate_backwards(data, 1990)
...
def test_custom_series_key():
    ...
    extrapolate_backwards(data, 2008, series_id=["Country", "Series"])
    years = sorted([d["Year"] for d in data])
