...
def test_extrapolate_forward_single_series():
    ...
    extrapolated_data = extrapolate_forward(data, 2003)
...
def test_no_forward_extrapolation_needed():
    ...
    extrapolated_data = extrapolate_forward(data, 2000)
...
def test_forward_custom_series_key():
    ...
    extrapolated_data = extrapolate_forward(data, 2012, series_id=["Country", "Series"])
    years = sorted([d["Year"] for d in extrapolated_data])
...
def test_extrapolate_forward_impute_only():
    ...
    result = extrapolate_forward(data, 2004, impute_only=True)
...
def test_extrapolate_forward_full_series():
    ...
    result = extrapolate_forward(data, 2002, impute_only=True)
    assert len(result) == 0
