from sspi_flask_app.api.resources.utilities import generate_series_levels


def test_generate_series_levels_default_keys():
    data = [
        {
    ...
    levels = generate_series_levels(data)
    assert len(levels) == 2
...
def test_generate_series_levels_exclude_additional_field():
    data = [
        {
    ...
    levels = generate_series_levels(data, exclude_fields=["Unit"])
    assert len(levels) == 1  # Only one unique level after excluding Unit
    assert "Unit" not in levels[0]


def test_generate_series_levels_custom_keys():
    data = [
        {
...
def test_generate_series_levels_duplicate_records_ignored():
    data = [
...
def test_generate_series_levels_handles_empty_list():
    levels = generate_series_levels([])
    assert levels == []


def test_generate_series_levels_ignores_list_values():
    data = [
        {"CountryCode": "CAN", "ItemCode": "SENLEM",
