const{endLabelPlugin}=require("./indicatorData");class DynamicLineChart{constructor(parentElement,IndicatorCode,CountryList=[]){this.parentElement=parentElement;this.IndicatorCode=IndicatorCode;this.CountryList=CountryList;this.pinnedArray=Array();this.initRoot();this.rigCountryGroupSelector();this.initChartJSCanvas();this.rigLegend();this.fetch().then(data=>{this.update(data);});}
initRoot(){this.root=document.createElement('div');this.root.classList.add('chart-section-dynamic-line');this.parentElement.appendChild(this.root);this.root.innerHTML=`<div class="chart-section-title-bar"><h2>Dynamic Indicator Data</h2><div class="chart-section-title-bar-buttons"><button class="draw-button">Draw 10 Countries</button><button class="showall-button">Show All</button><button class="hideunpinned-button">Hide Unpinned</button></div></div>`;this.rigTitleBarButtons();}
rigTitleBarButtons(){this.drawButton=this.root.querySelector('.draw-button');this.drawButton.addEventListener('click',()=>{this.showRandomN(10);});this.showAllButton=this.root.querySelector('.showall-button');this.showAllButton.addEventListener('click',()=>{this.showAll();});this.hideUnpinnedButton=this.root.querySelector('.hideunpinned-button');this.hideUnpinnedButton.addEventListener('click',()=>{this.hideUnpinned();});}
initChartJSCanvas(){this.canvas=document.createElement('canvas');this.canvas.id='dynamic-line-chart-canvas';this.canvas.width=400;this.canvas.height=200;this.context=this.canvas.getContext('2d');this.root.appendChild(this.canvas);this.chart=new Chart(this.context,{type:'line',options:{onClick:(event,elements)=>{elements.forEach(element=>{const dataset=this.chart.data.datasets[element.datasetIndex];this.togglePin(dataset);});},plugins:{legend:{display:false,},endLabelPlugin:{}},layout:{padding:{right:40}},scales:{x:{ticks:{color:'#bbb',},title:{display:true,text:'Year',color:'#bbb',font:{size:16}},},y:{ticks:{color:'#bbb',},beginAtZero:true,min:0,max:1,title:{display:true,text:'Indicator Score',color:'#bbb',font:{size:16}},}}},plugins:[endLabelPlugin]});}
rigCountryGroupSelector(){const container=document.createElement('div');container.id='country-group-selector-container';this.countryGroupContainer=this.root.appendChild(container);}
...
rigLegend(){const legend=document.createElement('legend');legend.classList.add('dynamic-line-legend');legend.innerHTML=`<div class="legend-title-bar"><h4 class="legend-title">Pinned Countries</h4><div class="legend-title-bar-buttons"><button class="saveprefs-button">Save Pins</button><button class="clearpins-button">Clear Pins</button></div></div><div class="legend-items"></div>`;this.savePrefsButton=legend.querySelector('.saveprefs-button');this.savePrefsButton.addEventListener('click',()=>{this.sendPrefs();});this.clearPinsButton=legend.querySelector('.clearpins-button');this.clearPinsButton.addEventListener('click',()=>{this.clearPins();});this.legend=this.root.appendChild(legend);this.legendItems=this.legend.querySelector('.legend-items');}
updateLegend(){this.legendItems.innerHTML='';this.pinnedArray.forEach((PinnedCountry)=>{this.legendItems.innerHTML+=`<div class="legend-item"><span>${PinnedCountry.CName}(<b style="color: ${PinnedCountry.borderColor};">${PinnedCountry.CCode}</b>)</span><button class="remove-button-legend-item"id="${PinnedCountry.CCode}-remove-button-legend">Remove</button></div>`;});this.removeButtons=this.legend.querySelectorAll('.remove-button-legend-item');this.removeButtons.forEach((button)=>{const CountryCode=button.id.split('-')[0];button.addEventListener('click',()=>{this.unpinCountryByCode(CountryCode);});});this.legendItems.innerHTML+=`<div class="legend-item"><button class="add-country-button">Add Country</button></div>`;this.addCountryButton=this.legend.querySelector('.add-country-button');this.addCountryButton.addEventListener('click',()=>{const search=new SearchDropdown(this.addCountryButton,this.chart.data.datasets,this);});}
async fetch(){const response=await fetch(`/api/v1/dynamic/line/${this.IndicatorCode}`);try{return response.json();}catch(error){console.error('Error:',error);}}
update(data){this.chart.data=data;this.chart.data.labels=data.labels;this.chart.data.datasets=data.data;this.chart.options.plugins.title=data.title;this.pinnedArray.push(...data.chartPreferences.pinnedArray);this.groupOptions=data.groupOptions;this.updatePins();this.updateLegend();this.updateCountryGroups();this.chart.update();}
updatePins(){if(this.pinnedArray.length===0){return;}
this.chart.data.datasets.forEach(dataset=>{if(this.pinnedArray.map(cou=>cou.CCode).includes(dataset.CCode)){dataset.pinned=true;dataset.hidden=false;}});this.chart.update();}
showAll(){this.pinnedOnly=false;console.log('Showing all countries');this.chart.data.datasets.forEach((dataset)=>{dataset.hidden=false;});this.chart.update({duration:0,lazy:false});}
showGroup(groupName){this.pinnedOnly=false;console.log('Showing group:',groupName);this.chart.data.datasets.forEach((dataset)=>{if(dataset.CGroup.includes(groupName)|dataset.pinned){dataset.hidden=false;}else{dataset.hidden=true;}});this.chart.update({duration:0,lazy:false});}
hideUnpinned(){this.pinnedOnly=true;console.log('Hiding all countries');this.chart.data.datasets.forEach((dataset)=>{if(!dataset.pinned){dataset.hidden=true;}});this.chart.update({duration:0,lazy:false});}
showRandomN(N=10){this.pinnedOnly=false;const activeGroup=this.groupOptions[this.countryGroupContainer.style.getPropertyValue('--selected-index')];let availableDatasetIndices=[];this.chart.data.datasets.filter((dataset,index)=>{if(dataset.CGroup.includes(activeGroup)){availableDatasetIndices.push(index);}});console.log('Showing',N,'random countries from group',activeGroup);this.chart.data.datasets.forEach((dataset)=>{if(!dataset.pinned){dataset.hidden=true;}});let shownIndexArray=availableDatasetIndices.sort(()=>Math.random()-0.5).slice(0,N);shownIndexArray.forEach((index)=>{this.chart.data.datasets[index].hidden=false;console.log(this.chart.data.datasets[index].CCode,this.chart.data.datasets[index].CName);});this.chart.update({duration:0,lazy:false});}
sendPrefs(){const activeGroup=this.groupOptions[this.countryGroupContainer.style.getPropertyValue('--selected-index')];fetch(`/api/v1/dynamic/line/${this.IndicatorCode}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pinnedArray:this.pinnedArray,activeGroup:activeGroup,pinnedOnly:this.pinnedOnly})});}
pinCountry(dataset){if(dataset.pinned){return;}
dataset.pinned=true;dataset.hidden=false;this.pinnedArray.push({CName:dataset.CName,CCode:dataset.CCode,borderColor:dataset.borderColor});this.updateLegend();}
unpinCountry(dataset){if(this.pinnedOnly){dataest.hidden=true;}
dataset.pinned=false;this.pinnedArray=this.pinnedArray.filter((item)=>item.CCode!==dataset.CCode);this.updateLegend();}
unpinCountryByCode(CountryCode){this.chart.data.datasets.forEach(dataset=>{if(dataset.CCode===CountryCode){dataset.pinned=false;this.pinnedArray=this.pinnedArray.filter((item)=>item.CCode!==dataset.CCode);}});this.updateLegend();}
togglePin(dataset){dataset.pinned=!dataset.pinned;if(dataset.pinned){this.pinnedArray.push({CName:dataset.CName,CCode:dataset.CCode,borderColor:dataset.borderColor});}else{this.pinnedArray=this.pinnedArray.filter((item)=>item.CCode!==dataset.CCode);}
this.updateLegend();}
clearPins(){this.pinnedArray=Array();this.chart.data.datasets.forEach(dataset=>{dataset.pinned=false;});this.updateLegend();}}
...
const meta=chart.getDatasetMeta(i);console.log(meta)
const lastPoint=meta.data[meta.data.length-1];const value=dataset.CCode;ctx.save();ctx.font='bold 14px Arial';ctx.fillStyle=dataset.borderColor;ctx.textAlign='left';ctx.fillText(value,lastPoint.x+5,lastPoint.y+4);ctx.restore();});}}
...
this.updateLegend()
this.chart.update()}
unpinCountry(dataset,hide=false){if(this.pinnedOnly||hide){dataset.hidden=true}
