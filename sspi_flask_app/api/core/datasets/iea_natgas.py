...
def clean_iea_natgas():
    ...
    intermediate_data = pd.DataFrame(clean_iea_data_altnrg(raw_data, "IEA_NATGAS"))
    ...
    # Filter only for NATGAS (natural gas) data
    intermediate_data = intermediate_data[intermediate_data["IntermediateCode"] == "NATGAS"]
    intermediate_data["IntermediateCode"] = "NATGAS"
    intermediate_data.astype({"Year": "int", "Value": "float"})
    
    # Assign DatasetCode to all records
    intermediate_data["DatasetCode"] = "IEA_NATGAS"
