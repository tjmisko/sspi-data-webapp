...
@dataset_collector("WID_NINCSH_POSTTAX")
def collect_wid_nincs_posttax(**kwargs):
    yield from collect_wid_data(**kwargs)


@dataset_cleaner("WID_NINCSH_POSTTAX")
def clean_wid_nincs_posttax():
    sspi_clean_api_data.delete_many({"DatasetCode": "WID_NINCSH_POSTTAX"})
    source_info = sspi_metadata.get_source_info("WID_NINCSH_POSTTAX")
    raw_data = sspi_raw_api_data.fetch_raw_data(source_info)
    
    # Target WID variables for post-tax national income shares
    target_variables = ["sdiincj992", "sdiincj999"]  # equal-split adults (adults, all ages)
    ...
    sspi_metadata.record_dataset_range(cleaned_data, "WID_NINCSH_POSTTAX")
