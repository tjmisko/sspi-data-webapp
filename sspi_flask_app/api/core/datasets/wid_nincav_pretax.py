...
@dataset_collector("WID_NINCAV_PRETAX")
def collect_wid_ninca_pretax(**kwargs):
    yield from collect_wid_data(**kwargs)


@dataset_cleaner("WID_NINCAV_PRETAX")
def clean_wid_ninca_pretax():
    sspi_clean_api_data.delete_many({"DatasetCode": "WID_NINCAV_PRETAX"})
    source_info = sspi_metadata.get_source_info("WID_NINCAV_PRETAX")
    raw_data = sspi_raw_api_data.fetch_raw_data(source_info)
    
    # Target WID variables for average pre-tax national income
    target_variables = ["aptincj992", "aptincj999"]  # equal-split adults (adults, all ages)
    ...
    sspi_metadata.record_dataset_range(cleaned_data, "WID_NINCAV_PRETAX")
