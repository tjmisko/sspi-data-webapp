...
def clean_iea_nclear():
    ...
    intermediate_data = pd.DataFrame(clean_iea_data_altnrg(raw_data, "IEA_NCLEAR"))
    ...
    # Filter only for NUCLEAR (nuclear) data
    intermediate_data = intermediate_data[intermediate_data["IntermediateCode"] == "NUCLEAR"]
    intermediate_data["IntermediateCode"] = "NCLEAR"
    intermediate_data.astype({"Year": "int", "Value": "float"})
    
    # Assign DatasetCode to all records
    intermediate_data["DatasetCode"] = "IEA_NCLEAR"
