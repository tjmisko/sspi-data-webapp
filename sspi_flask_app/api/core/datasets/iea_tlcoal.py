...
def clean_iea_tlcoal():
    ...
    intermediate_data = pd.DataFrame(clean_iea_data_altnrg(raw_data, "IEA_TLCOAL"))
    ...
    # Filter only for COAL (total coal) data
    intermediate_data = intermediate_data[intermediate_data["IntermediateCode"] == "COAL"]
    intermediate_data["IntermediateCode"] = "TLCOAL"
    intermediate_data.astype({"Year": "int", "Value": "float"})
    
    # Assign DatasetCode to all records
    intermediate_data["DatasetCode"] = "IEA_TLCOAL"
