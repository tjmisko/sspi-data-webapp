###############################################################
# Documentation: datasets/unsdg/unsdg_stkhlm/documentation.md #
###############################################################
...
@dataset_collector("UNSDG_STKHLM")
def collect_unsdg_terrst(**kwargs):
    yield from collect_sdg_indicator_data("12.4.1", **kwargs)

@dataset_cleaner("UNSDG_STKHLM")
def clean_unsdg_terrst():
    sspi_clean_api_data.delete_many({"DatasetCode": "UNSDG_STKHLM"})
    source_info = sspi_metadata.get_source_info("UNSDG_STKHLM")
    raw_data = sspi_raw_api_data.fetch_raw_data(source_info)
    extracted_unsdg_terrst = extract_sdg(raw_data)
    idcode_map = {"SG_HAZ_CMRSTHOLM": "UNSDG_STKHLM"}
    ...
    sspi_metadata.record_dataset_range(unsdg_stkhlm, "UNSDG_STKHLM")
    return unsdg_stkhlm
