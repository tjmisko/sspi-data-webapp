...
@dataset_collector("WID_WEALS_NET")
def collect_wid_weals_net(**kwargs):
    yield from collect_wid_data(**kwargs)


@dataset_cleaner("WID_WEALS_NET")
def clean_wid_weals_net():
    sspi_clean_api_data.delete_many({"DatasetCode": "WID_WEALS_NET"})
    source_info = sspi_metadata.get_source_info("WID_WEALS_NET")
    raw_data = sspi_raw_api_data.fetch_raw_data(source_info)
    
    # Target WID variables for net wealth shares
    target_variables = ["shwealj992", "shwealj999"]  # equal-split adults (adults, all ages)
    ...
    sspi_metadata.record_dataset_range(cleaned_data, "WID_WEALS_NET")
